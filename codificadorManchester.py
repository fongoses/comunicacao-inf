...
######################################################################
#
#Script de codificacao Manchester e NRZ.
#Requisitos: python (recomendavel versao 2.7) e gnuplot-x11
#
#Entrada: uma string de bits
#Saida: Graficos contendo a codificacao Manchester e NRZ correspondentes
#       a string de bits passada na entrada
#
#Exemplo:
#   ./codificadorManchester 10010101
#
######################################################################
...
def parseiaDadosEntradaManchester(sBits):
    ...
    for bit in sBits:    
        if int(bit) == 0:
            amp.extend(FASE_0_MANCH)
            tem.extend([i,i+0.5,i+0.5,i+1])        
            i+=1
        else:
            if int(bit) == 1:            
                amp.extend(FASE_1_MANCH)
                tem.extend([i,i+0.5,i+0.5,i+1])
                i+=1
            else:
...
PATH_ARQUIVO_TEMPORARIO_MANCH='tempmanch.txt'

arquivoTemporarioNrz = open(PATH_ARQUIVO_TEMPORARIO_NRZ,'w')
arquivoTemporarioManch = open(PATH_ARQUIVO_TEMPORARIO_MANCH,'w')
...
exibeGnuplot(PATH_ARQUIVO_TEMPORARIO_MANCH,stringBits,"Manchester",str(min(FASE_0_MANCH)-1),str(max(FASE_1_MANCH)+1))
exibeGnuplot(PATH_ARQUIVO_TEMPORARIO_NRZ,stringBits,"NRZ Unipolar",str(min(AMPLITUDE_0_NRZ)-1),str(max(AMPLITUDE_1_NRZ)+1))

#os.remove(PATH_ARQUIVO_TEMPORARIO_MANCH)
